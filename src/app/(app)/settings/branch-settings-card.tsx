...
// 銀行営業の判定基準（休眠日数・稼働判定・フォロー日数）の設定カード。
//
// これらは先方から確定値が届くまでの「仮値」であり、コードに直書きしない（§7-6）。
// 変更は app_settings テーブルに保存され、支店稼働の集計に即時反映される。
  ...
  return (
    <Card className="p-6">
      <div className="mb-3 flex flex-wrap items-center gap-2.5">
        ...
        <h2 className="font-bold">銀行営業の判定基準</h2>
        {isDefault && (
      ...
      <p className="mb-4 text-sm leading-relaxed text-slate-500 dark:text-slate-400">
        支店の休眠判定・稼働率の計算に使う日数です。確定した基準が決まったらここで変更してください。
        変更は支店稼働ダッシュボードと支店一覧に即時反映されます。
      </p>
      ...
      {loading ? (
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-2">
            {num("activeWindowDays", "稼働とみなす日数", "この日数以内に接点があれば稼働支店")}
            {num("recentMonths", "直近アポ数の集計月数", "支店一覧の「直近アポ」の対象期間")}
