...
| 🤖 商談ログ（AI） | 1マイクで録った文字起こしから、**話者分離・議事録・ToDo抽出・確度A/B/C判定・失注リスク**をAIが起こす。ToDoはワンタップでタスク化、確度は案件に反映 |
...
2. ダッシュボードの **SQL Editor** で `supabase/migrations/` 内の SQL を番号順にすべて実行
   ...
   **`0006_roles_rls.sql` でロールと RLS（ここで全ポリシーが入れ替わります）**、
   ...
   `0009_public_booking.sql` で公開予約リンクの匿名アクセスの最小化、
   `0010_meeting_logs.sql` で商談ログ（AI解析）のテーブルが作成されます）
...
商談ログの解析は **Claude API** を使います。サーバー環境変数に API キーを設定してください。

| 変数 | 内容 |
| --- | --- |
| `ANTHROPIC_API_KEY` | Claude API のキー。**`NEXT_PUBLIC_` は付けない**（付けるとブラウザに配布され、誰でも使えてしまいます） |

- モデルは `claude-opus-5`（`src/lib/server/anthropic.ts` の `ANALYSIS_MODEL` 1箇所で変更できます）
- 呼び出しは **サーバールート経由のみ**（`/api/ai/analyze-meeting`）。ブラウザから直接 Claude は呼びません
- このルートは**ログイン必須**です。未ログインのリクエストは 401 で弾き、AI を呼びません（APIキーの浪費と第三者の悪用を防ぐため）
- 未設定でも画面は開きます。「AIが未設定です」と表示され、解析ボタンだけが無効になります
- デモモード（Supabase 未接続）では Claude を呼ばず、サンプルの解析結果を返します。反映までの流れをそのまま確認できます
- 出力の形とプロンプトは `src/lib/meeting-analysis.ts` の1箇所に集約しています。確度の判定基準は「設定 > 確度の判定基準」の文言がそのまま渡ります

**料金の目安**: 商談1件（60分・約2万字）の解析でおおむね数十円程度です。件数が増える場合は `ANALYSIS_MODEL` の変更も検討してください。

### 6. Google カレンダー連携について
...
| `src/lib/meeting-analysis.test.ts` | AI解析のプロンプト組み立て・期限表現の日付化（「3営業日以内」など）・タスク化対象の絞り込み |
